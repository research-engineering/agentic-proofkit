...
- `docs/release-process.md` for release-channel evidence and publication
  process;
- `docs/specs/**/requirements.v2.json` for durable machine requirements;
...
|---|---|---|---|
| BLOCKED | SOURCE-CUTOVER-01 | Migrate self-hosted requirement sources only after one codec, the typed v2 model, changed-rule structural contracts, and the complete evidence counterfeit corpus pass their gates. | The `REQ-PROOFKIT-QUALITY-010` execution-backed command-oracle closure and change-bound `SCHEMA-01` gates pass; a digest-bound clause ledger proves representation-only equality or owner-reviewed semantic decomposition for every legacy requirement; all bindings/scenarios/contracts/context/diff/graph/browser owners cut over atomically; v1 admission and the losing codec are removed; active-v1 inventory is zero. Full elaboration of unchanged nested rules remains mandatory under `SCHEMA-01` after this cutover. |
...
| NEXT | SOURCE-CONVERSION-01 | Design and implement explicit flat/grouped source import and export through one semantic model, with a justified extension boundary for a possible future DSL adapter. | A reviewed design and plan define versioned format/domain/capability contracts, exact versus resolved-value preservation, ownership and provenance reconstruction, deterministic bounded diagnostics, and refusal of unsupported or unrequested loss. Independent bidirectional tests and installed CLI/carrier witnesses prove the supported conversions without duplicated semantic validators, inferred shared policy or automatic dual-source synchronization. Decide any legacy-v1 import role explicitly; a default-off old runtime is not the solution. Future DSL admission is documented, but implementing a DSL or speculative plugin framework is outside this task. |
