...
| NEXT | TYPESCRIPT-ASI-01 | Close consolidated audit F13 without adding more unsound statement-boundary heuristics to the current TypeScript public API scanner. | Select a bounded syntax owner or explicitly narrow the admitted source grammar; preserve valid exported declarations and reject unsupported forms before comparing manifests. Pinned TypeScript compiler and runtime oracles plus whole-CLI positive and negative manifests must cover semicolonless declarations, expression continuations, comments, strings, templates, Unicode line terminators, and a resource-bound corpus. No new runtime dependency is admitted without proving packaging and maintenance cost. |
