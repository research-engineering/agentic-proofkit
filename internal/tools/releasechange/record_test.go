...
var currentBreakingChanges = []Change{
	...
	{ChangeID: "proofkit.typescript-public-api.exports", Summary: "TypeScript public API verification now compares static ESM source declarations in a restricted .ts/.mts subset; CommonJS .cts and enum or namespace declarations need repository-owned native witnesses. Manifests claiming truncated dollar-suffixed names or exports from a later local declaration fail. Unresolved runtime re-exports, code identifiers module/exports, invalid type names, duplicate type-only modifiers, unsupported type-only import attributes, and ambiguous .mts expression-arrow syntax fail closed; direct semicolonless exports remain supported. A passing inventory does not prove pinned TypeScript compiler syntax or type validity. The sourceGrammar marker advances to v2."},
}
