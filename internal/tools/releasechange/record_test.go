...
var currentBreakingChanges = []Change{
	...
	{ChangeID: "proofkit.deployment-endpoint.admission", Summary: "Stable endpoints using case, trailing-root-dot or IDNA-equivalent variants of a declared temporary DNS suffix and temporary endpoints with impossible RFC3339 UTC calendar values now fail admission. An unrelated hostname ending in the same text without a DNS label boundary no longer falsely matches; announced historical leap seconds remain valid."},
	{ChangeID: "proofkit.typescript-public-api.exports", Summary: "TypeScript public API manifests that truncate dollar-suffixed identifiers or claim exports from a later local declaration now fail verification. Unresolved named runtime re-exports and const enums fail closed; valid direct semicolonless exports remain supported. The machine-readable sourceGrammar marker advances to v2."},
}
...
var currentMigrationSteps = []string{
	...
	"Replace unresolved named runtime re-exports with direct exports or defer adoption until an owner-admitted target resolver exists; use explicit type-only re-exports only with separate target-symbol evidence.",
...
func currentExpectedReleaseNotes() string {
	...
	lines = append(lines,
		...
		"- Named runtime re-exports and const enum declarations are not admitted; explicit type-only re-exports do not prove target-symbol existence.",
