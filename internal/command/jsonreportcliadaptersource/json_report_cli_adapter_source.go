...
func TypeScriptSource() string {
	return strings.NewReplacer(
		...
		"__PROOFKIT_COMMAND_ROUTE_TOKEN_PATTERN__", strings.ReplaceAll(commandroute.TokenPattern, "/", `\/`),
		"__PROOFKIT_COMMAND_ROUTE_ERROR__", strconv.Quote(fmt.Sprintf("agentic-proofkit command route must contain %d to %d canonical tokens", commandroute.MinimumTokens, commandroute.MaximumTokens)),
	).Replace(typeScriptSourceTemplate)
}
...
const typeScriptSourceTemplate = `/* Generated by agentic-proofkit json-report-cli-adapter-source.
...
function admitCommandRoute(commandRoute: string): readonly string[] {
  const tokens = commandRoute.split(__PROOFKIT_COMMAND_ROUTE_SEPARATOR__);
  if (tokens.length < __PROOFKIT_COMMAND_ROUTE_MINIMUM__ || tokens.length > __PROOFKIT_COMMAND_ROUTE_MAXIMUM__ || tokens.some((token) => !/__PROOFKIT_COMMAND_ROUTE_TOKEN_PATTERN__/u.test(token))) {
