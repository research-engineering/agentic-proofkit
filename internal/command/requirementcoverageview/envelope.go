...
func agentEnvelope(view map[string]any, renderer cliexec.Renderer) map[string]any {
	...
	return agentenvelope.Build(agentenvelope.Input{
		ActionPlan: []map[string]any{
			{
				...
				"instruction":  "Use the coverage view to identify requirements, commands, and test surfaces lacking declared falsification routes.",
				"nonClaims":    []any{"This action does not approve merge or create tests."},
				"owner":        "consumer_repository",
				"phase":        "review",
				"rationale":    "Route declarations establish mapping only; execution-backed semantic evidence requires separate owners.",
		...
		ContextRefs: []map[string]any{
			...
			contextRef("proofkit.requirement-coverage-view.owner-invariants", "/ownerInvariantCoverage", "Per-owner-invariant route-mapping states."),
			contextRef("proofkit.requirement-coverage-view.requirements", "/requirementCoverage", "Per-requirement route-mapping states."),
			contextRef("proofkit.requirement-coverage-view.commands", "/commandCoverage", "Per-command declared-route states."),
		},
		...
		NonClaims: []string{
			...
			"Requirement coverage envelopes do not prove selector resolution, oracle quality, freshness, trust, sensitivity, or assurance closure.",
		},
