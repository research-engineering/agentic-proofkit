...
func TestPlanIdentityBindsIntentAndInventory(t *testing.T) {
	inventory := adoptionInventory(t)
	fresh, err := Build(IntentFresh, inventory, "")
	if err != nil {
		t.Fatalf("Build(fresh) error = %v", err)
	}
	baseline, err := Build(IntentCodeBaseline, inventory, "")
	if err != nil {
		t.Fatalf("Build(baseline) error = %v", err)
	}
	if fresh.PlanID == baseline.PlanID {
		t.Fatal("plan identity did not bind source-trust intent")
	}

	changedRoot := t.TempDir()
	if err := os.WriteFile(filepath.Join(changedRoot, "README.md"), []byte("# Changed\n"), 0o600); err != nil {
		t.Fatalf("WriteFile() error = %v", err)
	}
	changedInventory, err := repositoryinventory.Scan(context.Background(), changedRoot)
	if err != nil {
		t.Fatalf("repositoryinventory.Scan() error = %v", err)
	}
	changed, err := Build(IntentFresh, changedInventory, "")
	if err != nil {
		t.Fatalf("Build(changed inventory) error = %v", err)
	}
	if fresh.PlanID == changed.PlanID {
		t.Fatal("plan identity did not bind repository inventory")
	}
}
...
func TestTextProjectionPreservesJSONPlanSemantics(t *testing.T) {
	...
	for _, nonClaim := range collectAuthorityNonClaims(plan.JSONValue()) {
		if strings.Count(text, nonClaim) != 1 {
			t.Fatalf("text projection must contain non-claim exactly once %q", nonClaim)
		}
	}
...
func TestAdoptionPlanOutputAndTextBoundsAreExact(t *testing.T) {
	plan, err := Build(IntentFresh, adoptionInventory(t), "")
	if err != nil {
		t.Fatalf("Build() error = %v", err)
	}
	...
	if err := validateOutputByteLimit(plan, len(encoded)); err != nil {
		t.Fatalf("exact plan output byte limit rejected: %v", err)
	}
	if err := validateOutputByteLimit(plan, len(encoded)-1); err == nil {
		t.Fatal("one-over plan output survived the byte limit")
	}

	exactByteLines := []TextLine{{Label: "L", Value: strings.Repeat("x", MaximumTextBytes-4)}}
	if text, err := renderTextWithinLimits(exactByteLines, MaximumTextBytes, MaximumTextLines); err != nil || len(text) != MaximumTextBytes {
		t.Fatalf("exact text byte limit: bytes=%d error=%v", len(text), err)
	}
	overByteLines := []TextLine{{Label: "L", Value: strings.Repeat("x", MaximumTextBytes-3)}}
	if _, err := renderTextWithinLimits(overByteLines, MaximumTextBytes, MaximumTextLines); err == nil {
		t.Fatal("one-over text survived the byte limit")
	}
	exactLineCount := make([]TextLine, MaximumTextLines)
	for index := range exactLineCount {
		exactLineCount[index] = TextLine{Label: "L"}
	}
	if _, err := renderTextWithinLimits(exactLineCount, MaximumTextBytes, MaximumTextLines); err != nil {
		t.Fatalf("exact text line limit rejected: %v", err)
	}
	if _, err := renderTextWithinLimits(append(exactLineCount, TextLine{Label: "L"}), MaximumTextBytes, MaximumTextLines); err == nil {
		t.Fatal("one-over text survived the line limit")
	}
}

func collectAuthorityNonClaims(value any) []string {
	result := []string{}
	var visit func(any)
