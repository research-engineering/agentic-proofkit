...
var guidanceTable = [...]Slot{
	...
	{Order: 13, SlotID: "output_bounds", ApplicabilityClass: ApplicabilityAlways, Question: "Which report-visible output channels and bounds apply?", RequiredConsumerDecision: "For each declared channel, select an exact byte cap and overflow behavior; record every absent channel as not_applicable.", CompletionCriterion: "Each declared channel passes at its boundary and fails one-over before partial output; absent channels emit no output."},
	...
	{Order: 18, SlotID: "receipt_identity", ApplicabilityClass: ApplicabilityAlways, Question: "Which operation and channel identities must the receipt bind?", RequiredConsumerDecision: "Select operation, subject, and witness identities; bind each declared argv, input, and output channel by digest and mark each absent channel not_applicable.", CompletionCriterion: "The receipt binds every applicable identity, rejects any one-field substitution, and carries no identity for an absent channel."},
