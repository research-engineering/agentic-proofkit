...
var commandCoverageRoutes = map[string][]commandCoverageRoute{
	"transaction-inspect-residue":    {directCLIRoute("internal/app/transaction_residue_command_test.go", "TestTransactionInspectResidueCLI", semanticRouteProof("transaction_residue.inspect_whole_cli"), "Residue inspection must remain read-only, distinguish absent from eligible unknown bytes, reject recognized journals and disclose only closed observation facts.")},
	"transaction-quarantine-residue": {directCLIRoute("internal/app/transaction_residue_command_test.go", "TestTransactionQuarantineResidueCLI", semanticRouteProof("transaction_residue.quarantine_whole_cli"), "Residue quarantine must bind the exact observation, relocate evidence without changing bytes or retained receipts, reject stale tokens and never move a new active directory on retry.")},
	"integration-apply":              {directCLIRoute("internal/app/agent_integration_lifecycle_command_test.go", "TestIntegrationApplyCLI", semanticRouteProof("agent_integration.apply_whole_cli"), "Integration apply must bind both reviewed identities, preserve local edits, install exact generated bytes, replay safely and remove only selected managed targets.")},
	"integration-plan":               {directCLIRoute("internal/app/agent_integration_lifecycle_command_test.go", "TestIntegrationPlanCLI", semanticRouteProof("agent_integration.plan_whole_cli"), "Integration plan must classify the selected operation and pair state without mutation or claiming an absent installation can be updated.")},
	"integration-recover":            {directCLIRoute("internal/app/agent_integration_lifecycle_command_test.go", "TestIntegrationRecoverCLI", semanticRouteProof("agent_integration.recover_whole_cli"), "Integration recovery must preserve the historical native transaction outcome without assuming current tool source or permitting rollback of a committed transaction.")},
	...
	"view":                           {directCLIRoute("internal/app/project_view_command_test.go", "TestProjectViewCLI", semanticRouteProof("project_view.whole_cli"), "Project view must consume only an explicit complete project, emit a bounded workspace plan and reject stale input without mutating source or promoting verification.")},
