...
var commandCoverageRoutes = map[string][]commandCoverageRoute{
	...
	"requirement-context-slice":             {requiredInputAdmissionRoute, packageFalsifierRoute("internal/command/requirementcontext/requirementcontext_test.go", "TestSliceRejectsTamperedSnapshotAndUnknownNode", semanticRouteProof("requirementcontext.slice_rejects_tampered_snapshot_and_unknown_node", commandCoverageExpectedPublicOutcome), "Requirement context slicing must reject stale snapshot identity and unknown explicit semantic targets.")},
	...
	"requirement-semantic-diff":                {requiredInputAdmissionRoute, packageFalsifierRoute("internal/command/requirementdiff/requirementdiff_test.go", "TestBuildClassifiesOwnerAwareRequirementChanges", semanticRouteProof("requirementdiff.build_classifies_owner_aware_requirement_changes", commandCoverageExpectedPublicOutcome), "Requirement semantic diff must compare admitted requirement fields by owner-declared field class instead of textual order.")},
	...
	"requirement-traceability-graph":           {requiredInputAdmissionRoute, packageFalsifierRoute("internal/command/requirementgraph/requirementgraph_test.go", "TestBuildKeepsTraceabilityEvidencePlanesDistinct", semanticRouteProof("requirementgraph.build_keeps_traceability_evidence_planes_distinct", commandCoverageExpectedPublicOutcome), "Requirement traceability graph must keep specification, proof, code traceability, and native execution evidence planes distinct.")},
