...
// Static routes remain declaration-only candidates. The command-oracle owner
// separately joins them to source-bound cooperative runtime evidence.
...
var commandCoverageRoutes = map[string][]commandCoverageRoute{
	"integration-source":         {directCLIRoute("internal/app/agent_integration_command_test.go", "TestIntegrationSourceCLI", semanticRouteProof("agent_integration.source_whole_cli"), "Integration source must emit exact bounded portable bytes through JSON and text without reading caller input or accepting implicit tool selection.")},
	"integration-check":          {directCLIRoute("internal/app/agent_integration_command_test.go", "TestIntegrationCheckCLI", semanticRouteProof("agent_integration.check_whole_cli"), "Integration check must classify actual fixed-path bytes without writing files or treating missing, stale, or invalid content as current.")},
	...
	"change-workflow-plan": {
		...
		directCLIRoute("internal/app/agent_workflow_command_test.go", "TestAgentWorkflowCLITruthTable", semanticRouteProof("agent_workflow_command.agent_workflow_cli_truth_table"), "Change workflow planning must preserve the public JSON, text, envelope, rejection, help, and color contract across the full CLI truth table."),
		packageFalsifierRoute("internal/command/changeworkflowplan/state_test.go", "TestWorkflowStatePredicates", semanticRouteProof("changeworkflowplan.workflow_state_predicates"), "Change workflow planning must implement the total 29-state checkpoint relation without inventing repository state."),
		packageFalsifierRoute("internal/command/changeworkflowplan/state_test.go", "TestWorkflowCheckpointPredicates", semanticRouteProof("changeworkflowplan.workflow_checkpoint_predicates"), "Change workflow planning must fail closed for invalid checkpoints and impossible completion histories."),
	},
	...
	"native-evidence-guidance": {
		directCLIRoute("internal/app/agent_workflow_command_test.go", "TestAgentWorkflowCLITruthTable", semanticRouteProof("agent_workflow_command.native_evidence_guidance_cli_truth_table"), "Native evidence guidance must preserve the public JSON, text, rejection, help, and color contract across the CLI truth table."),
		packageFalsifierRoute("internal/command/nativeevidenceguidance/guidance_test.go", "TestGuidanceSlotPredicates", semanticRouteProof("nativeevidenceguidance.guidance_slot_predicates"), "Native evidence guidance must expose the complete fixed 22-slot repository-neutral evidence vocabulary in canonical order."),
		packageFalsifierRoute("internal/command/nativeevidenceguidance/guidance_test.go", "TestGuidancePurityPredicates", semanticRouteProof("nativeevidenceguidance.guidance_purity_predicates"), "Native evidence guidance must remain deterministic and return fresh caller-owned projections without ambient authority."),
	},
	"next": {
		directCLIRoute("internal/app/project_status_command_test.go", "TestNextCLI", semanticRouteProof("project_status_command.next_whole_cli"), "Project next must preserve one owner-admitted non-executable action across JSON, text, color, and argument-admission paths."),
		packageFalsifierRoute("internal/command/projectstatus/projectstatus_test.go", "TestEvaluateTotalStateActionTable", semanticRouteProof("projectstatus.evaluate_total_state_action_table"), "Project next must remain a total one-action projection for every admitted project state without claiming execution or completion."),
	},
	...
	"status": {
		directCLIRoute("internal/app/project_status_command_test.go", "TestStatusCLI", semanticRouteProof("project_status_command.status_whole_cli"), "Project status must preserve owner-admitted bounded classification across JSON, text, color, and pre-I/O argument admission paths."),
		packageFalsifierRoute("internal/command/projectstatus/inspect_test.go", "TestInspectClassifiesMaterializedProjectWithoutApplicationWrites", semanticRouteProof("projectstatus.inspect_classifies_materialized_project_without_application_writes"), "Project status must classify absent, admitted, and stale materialized projects without performing repository mutation operations or disclosing repository paths."),
		packageFalsifierRoute("internal/command/projectstatus/inspect_test.go", "TestInspectCohortValidationClosesCleanEpochABA", semanticRouteProof("projectstatus.inspect_cohort_validation_closes_clean_epoch_aba"), "Project status must reject a clean-state ABA when manifest or child content changes between its bounded observation passes."),
	},
...
func (route commandCoverageRoute) sourceOracleMarker(command string) string {
	binding := route.sourceOracleBinding(command)
	raw, err := stablejson.MarshalLayout(map[string]any{
		"commandRef":                 binding.CommandRef,
		"expectedPublicOutcome":      binding.ExpectedPublicOutcome,
		"falsifierId":                binding.FalsifierID,
		"negativeCaseId":             binding.NegativeCaseID,
		"oracleId":                   binding.OracleID,
		"oracleKind":                 binding.OracleKind,
		"schemaVersion":              binding.SchemaVersion,
		"selector":                   binding.Selector,
		"semanticRouteInvariantId":   binding.SemanticRouteInvariantID,
		"sourcePath":                 binding.SourcePath,
		"testId":                     binding.TestID,
		"wrongImplementationClassId": binding.WrongImplementationClass,
...
func (route commandCoverageRoute) semanticProofProblem() string {
	...
	if strings.TrimSpace(route.rationale) == "" {
		return "semantic coverage route requires a route-specific expected public outcome"
	}
