...
var commandCoverageRoutes = map[string][]commandCoverageRoute{
	...
	"next": {
		directCLIRoute("internal/app/project_status_command_test.go", "TestProjectStatusCLI", semanticRouteProof("project_status_command.next_whole_cli"), "Project next must preserve one owner-admitted non-executable action across JSON, text, color, and argument-admission paths."),
	...
	"status": {
		directCLIRoute("internal/app/project_status_command_test.go", "TestProjectStatusCLI", semanticRouteProof("project_status_command.status_whole_cli"), "Project status must preserve owner-admitted bounded classification across JSON, text, color, and pre-I/O argument admission paths."),
		packageFalsifierRoute("internal/command/projectstatus/inspect_test.go", "TestInspectClassifiesMaterializedProjectWithoutMutation", semanticRouteProof("projectstatus.inspect_classifies_materialized_project_without_mutation"), "Project status must classify absent, admitted, and stale materialized projects without mutating transaction state or disclosing repository paths."),
