...
| Does a TypeScript package public API match a caller-owned manifest? | `agent-route` with `goal: "verify_typescript_public_api"` and explicit `typescript_public_api_manifest` plus `typescript_public_api_repo_root`, then `typescript-public-api-surfaces --repo-root <caller-selected-root>` | The manifest must name each referenced `package.json`, sorted-unique export conditions, and a non-JSX ESM `.ts` or `.mts` `sourcePath` whose canonical target has the same admitted extension class. The bounded parser admits only the fail-closed source-declaration grammar in `proofkit/cli-contract.v2.json`; CommonJS `.cts` requires a repository-owned native witness. Unresolved named runtime re-exports and const enums are rejected; explicit type-only re-exports do not prove target-symbol existence. The command does not parse unrestricted TypeScript or TSX, infer conventional layouts, or prove compiler output provenance, checkout freshness, package-manager truth, or merge readiness. |
