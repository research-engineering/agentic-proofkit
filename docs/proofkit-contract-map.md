...
| Document lifecycle, design docs, generated views, or implementation plans need authority classification. | `document-lifecycle-boundary` | Treat lifecycle records as caller-owned metadata. Design docs and implementation plans are not package or merge authority unless a separate owner admits them. |
