...
| Does the selective plan need a text or secret scan? | Prefer `scanObligation` with `commandOwnership: "proofkit_text_policy"` for `text-policy` or `commandOwnership: "caller_owned_external"` for an external scanner; legacy `secretScan` is a compatibility alias only. | Stop before treating `text-policy` as a repository-wide secret scanner. Proofkit admits scan obligations but does not execute scanner commands or prove repository-wide credential absence. |
