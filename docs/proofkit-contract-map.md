...
| Does a TypeScript package public API match a caller-owned manifest? | `agent-route` with `goal: "verify_typescript_public_api"` and explicit `typescript_public_api_manifest` plus `typescript_public_api_repo_root`, then `typescript-public-api-surfaces --repo-root <caller-selected-root>` | The manifest must name each referenced `package.json`, sorted-unique export conditions, and a non-JSX ESM `sourcePath` with exact lowercase `.ts` or `.mts` extension whose canonical target has the same admitted extension class. The bounded parser compares a static declaration inventory within the explicit grammar in `proofkit/cli-contract.v2.json`; CommonJS `.cts`, enum declarations, and namespace declarations require a repository-owned native witness. Unresolved named runtime re-exports are rejected; explicit type-only re-exports do not prove target-symbol existence. A passing report does not certify pinned TypeScript compiler syntax, types, or built-module exports. The command does not parse unrestricted TypeScript or TSX, infer conventional layouts, or prove checkout freshness, package-manager truth, or merge readiness. |
