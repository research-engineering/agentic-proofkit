...
In a command direction, `childDefinitionBindings` binds `pathSegments` to the
one shipped `definitionRef` and its digest; `*` means each array element.
`pathRelations` declares a native-owned derived path rule, while
`handoffClauses` scopes the changed anchor schema to the submitted one-shot
variant. These records describe named changed children only. A root-shape
definition still does not claim the semantics of other nested fields; native
admission and runtime witnesses remain mandatory.
...
|---|---|---|---|---|---|
| Repository transaction maintenance | `transaction inspect-residue`, `transaction quarantine-residue` | explicit repository root; exact inspected SHA256 observation for quarantine; no JSON input or stdin | read-only absent/eligible observation, exclusive observation-bound whole-directory relocation, destination-first retry refusal, closed non-disclosing output and preserved terminal evidence | disposition authorization, unresolved unsafe state, archive retention/disposal, historical effect assessment | version-1 inspection or relocation observation only; no guessed transaction identity, transaction receipt, rollback, deletion or recovery claim |
...
| Temporary external design, implementation-plan, PR, code, or test observations may contain durable requirements. | `requirement-authoring-plan` | Treat output as candidate-only; stop before writing `requirements.v2.json`, retaining temporary documents, or claiming requirement meaning. |
...
| Does a TypeScript package public API match a caller-owned manifest? | `agent-route` with `goal: "verify_typescript_public_api"` and explicit `typescript_public_api_manifest` plus `typescript_public_api_repo_root`, then `typescript-public-api-surfaces --repo-root <caller-selected-root>` | The manifest must name each referenced `package.json`, sorted-unique export conditions, and a non-JSX `.ts`, `.mts`, or `.cts` `sourcePath` whose canonical target has the same admitted extension class. The bounded scanner accepts only the export grammar in `proofkit/cli-contract.v2.json`; semicolonless statement boundaries can misclassify local names until `TYPESCRIPT-ASI-01` is closed, so require a repository-owned compiler witness for those sources. It does not parse unrestricted TypeScript or TSX, infer conventional layouts, or prove compiler output provenance, checkout freshness, package-manager truth, or merge readiness. |
| A native run or blocked/not-run attempt needs a receipt and bundle input. | `proof-receipt-admission --help` explains required observations and digest subjects; `spec-proof-bundle-admission --help` connects the exact receipt report and original input to already reviewed binding and scheduler inputs. | Required null operands deliberately reject. The guides do not execute witnesses or create scheduler policy. Structural receipt/bundle success does not imply native success, currentness, authentication, trust or merge approval. |
